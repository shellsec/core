	...
	public function testAcceptRejectShare($method, $target, $targetExists, $expectedState) {
		...
		if ($method === 'acceptShare') {
			...
			$this->eventDispatcher->expects($this->exactly(2))
				->method('dispatch')
				->withConsecutive(
					[$this->equalTo('share.beforeaccepted'), $this->equalTo(new GenericEvent(null, ['share' => $userShare]))],
					[$this->equalTo('share.afteraccepted'), $this->equalTo(new GenericEvent(null, ['share' => $userShare]))]
				);
		} else {
			...
			$this->eventDispatcher->expects($this->exactly(2))
				->method('dispatch')
				->withConsecutive(
					[$this->equalTo('share.beforerejected'), $this->equalTo(new GenericEvent(null, ['share' => $userShare]))],
					[$this->equalTo('share.afterrejected'), $this->equalTo(new GenericEvent(null, ['share' => $userShare]))]
				);
		}
